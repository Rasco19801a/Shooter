...
export function attachTouchControls(root, state){
  ...
  function onMoveL(e){ if(!startL) return; const t=e.changedTouches[0]; const dx=t.clientX-startL.x, dy=t.clientY-startL.y; const max=80; const nx=clamp(dx/max,-1,1), ny=clamp(dy/max,-1,1); const p=state.player; const f={x:Math.cos(p.dir), y:Math.sin(p.dir)}, r={x:Math.cos(p.dir+Math.PI/2), y:Math.sin(p.dir+Math.PI/2)}; state.moveVec.x = f.x*(-ny) + r.x*nx; state.moveVec.y = f.y*(-ny) + r.y*nx; e.preventDefault(); }
  ...
  function onMoveR(e){ if(!R.active) return; const t=e.changedTouches[0]; const dx = t.clientX - R.ox; const dy = t.clientY - R.oy; let nx = clamp(dx/LOOK_RADIUS, -1, 1); let ny = clamp(dy/LOOK_RADIUS, -1, 1); if (Math.abs(nx) < DEADZONE) nx = 0; else nx = (nx - Math.sign(nx)*DEADZONE) / (1 - DEADZONE); if (Math.abs(ny) < DEADZONE) ny = 0; else ny = (ny - Math.sign(ny)*DEADZONE) / (1 - DEADZONE); state.turnStickX = nx; state.turnStickY = -ny; e.preventDefault(); }
